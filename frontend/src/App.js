...
function App() {
  ...
  const [pantallaCreativa, setPantallaCreativa] = React.useState(false);
  ...
  return (
    <>
      ...
      {mode === 'amigos' && (
        pantallaCreativa ? (
          <PantallaTexto onVolver={() => setPantallaCreativa(false)} />
        ) : (
              ...
              !showMapaAmigos ? (
                ...
                showMapaAmigos === 'success' ? (
                  <div style={{ marginTop: '-20px', color: '#e6f7ff', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <button
                      className="right-btn"
                      style={{ fontSize: 20, padding: '14px 0', marginTop: 24, width: 320, alignSelf: 'center' }}
                      onClick={() => setPantallaCreativa(true)}
                    >
                      Usa tu creatividad
                    </button>
                  </div>
